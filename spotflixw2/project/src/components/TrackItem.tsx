...
const TrackItem: React.FC<TrackItemProps> = ({
  ...
  const handleLike = () => {
    const likedTracks: string[] = JSON.parse(
      localStorage.getItem("likedTracks") || "[]"
    );
    let updatedLikes: string[];

    if (liked) {
      updatedLikes = likedTracks.filter((id) => id !== track.id);
    } else {
      updatedLikes = [...likedTracks, track.id];
    }
  ...
  return (
    <div className="group flex items-center space-x-4 p-2 rounded-lg hover:bg-gray-800 transition-colors">
      ...
      <div className="flex items-center space-x-4">
        ...
        >
          {liked ? <HeartFilled fill="white" size={16} /> : <Heart size={16} />}
        </button>
